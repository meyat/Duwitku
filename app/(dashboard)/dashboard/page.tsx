...
import { formatDate, daysUntil, safePercentage } from "@/lib/utils";
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Ringkasan kondisi keuanganmu</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {loading || !data ? (
          ...
          <>
            <SummaryCard label="Saldo Saat Ini" value={data.balance} icon={Wallet} tone="primary" currency={currency} />
            <SummaryCard label="Pemasukan Bulan Ini" value={data.monthIncome} icon={TrendingUp} tone="success" currency={currency} />
            <SummaryCard label="Pengeluaran Bulan Ini" value={data.monthExpense} icon={TrendingDown} tone="danger" currency={currency} />
            <SummaryCard
              ...
              currency={currency}
            />
            <SummaryCard label="Tabungan Terkumpul" value={data.savingsCollected} icon={PiggyBank} tone="primary" currency={currency} />
            <SummaryCard label="Total Utang" value={data.totalDebt} icon={HandCoins} tone="danger" currency={currency} />
            <SummaryCard label="Total Piutang" value={data.totalReceivable} icon={HandCoins} tone="success" currency={currency} />
            <SummaryCard label="Nilai Investasi" value={data.totalInvestmentValue} icon={LineChart} tone="warning" currency={currency} />
          </>
        )}
      </div>
        ...
        <Card className="border-warning/30 bg-warning-soft">
          <CardContent className="py-3 flex items-center gap-3">
            ...
            <p className="text-sm text-foreground">
              {"Ada"}{" "}
              <span className="font-semibold">{data.dueSoonBillsCount}</span>{" "}
                ...
                <div key={tx.id} className="flex items-center justify-between py-2.5 border-b border-border last:border-0">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {tx.category?.name ?? "Tanpa kategori"} · {formatDate(tx.date)}
                    </p>
                  </div>
                  <span
