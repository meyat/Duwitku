...
export function TransactionForm({ initial }: { initial?: Partial<TransactionFormValues> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="wallet">Wallet (opsional)</Label>
        <Select id="wallet" value={values.walletId} onChange={(e) => update("walletId", e.target.value)}>
          <option value="">Tanpa wallet spesifik</option>
          {wallets.map((w) => (
            <option key={w.id} value={w.id}>
              {w.name}
              {w.type === "BANK" && w.bankName ? ` (${w.bankName})` : ""}
            </option>
          ))}
        </Select>
      </div>
